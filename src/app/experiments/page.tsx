...
const Item = ({ flag }: { flag: (typeof featureFlags)[number] }) => {
  const [value, setValue] = useLocalStorage(flag.key, String(flag.defaultValue));
  ...
  return (
    ...
    >
      ...
      <div className={`relative shrink-0 w-11 h-6 rounded-full ${value === 'true' ? 'bg-orange-500' : 'bg-gray-300'}`}>
        <span
          className={`absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full shadow ${value === 'true' ? 'translate-x-5' : ''}`}
        />
      </div>
    </button>
  );
  ...
  return (
    <div className="min-h-screen bg-cream-50 py-16 px-6">
      <div className="max-w-xl mx-auto">
        ...
        <h1 className="text-3xl font-black text-brown-900 mt-6 mb-2">Experiments</h1>
        <p className="text-brown-600 mb-10">Toggle experimental features. Changes apply immediately.</p>
